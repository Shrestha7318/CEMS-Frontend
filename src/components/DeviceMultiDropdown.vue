  ...
  <div class="relative inline-block text-left" ref="root">
    ...
    >
      ...
      <svg class="h-4 w-4 opacity-70" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
        <path fill-rule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 11.157l3.71-3.93a.75.75 0 011.08 1.04l-4.24 4.5a.75.75 0 01-1.08 0l-4.24-4.5a.75.75 0 01.02-1.06z" clip-rule="evenodd"/>
      </svg>
    </button>
    ...
    >
      ...
      <ul class="max-h-64 overflow-auto py-1">
        <li class="px-2 py-1">
          <label class="flex items-center gap-2 text-sm px-2 py-1 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer">
        ...
        >
          <label class="flex items-center gap-2 text-sm px-2 py-1 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer">
            ...
            />
            <span class="truncate">{{ d.name }} <span class="text-xs text-gray-500">({{ d.id }})</span></span>
          </label>
        </li>
      ...
      <div class="flex items-center justify-between gap-2 p-2 border-t border-gray-100 dark:border-gray-800">
        <button class="text-xs px-3 py-1.5 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800" @click="clear">Clear</button>
        <button class="text-xs px-3 py-1.5 rounded-lg border border-gray-300 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800" @click="open=false">Done</button>
      </div>
    </div>
  </div>
</template>
